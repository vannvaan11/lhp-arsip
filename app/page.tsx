    ...
    return (
      <div className="relative min-h-screen w-full flex items-center justify-center p-4 bg-slate-900">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 bg-white/10 backdrop-blur-2xl p-10 rounded-[40px] shadow-2xl w-full max-w-md border border-white/20 text-center">
          ...
          <p className="text-white/60 mb-10 text-sm italic font-medium uppercase tracking-widest">Digital Archives Irban III</p>
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="password" placeholder="Kode Akses Khusus" className="w-full p-5 rounded-3xl border-none outline-none bg-white/10 text-white text-center font-bold placeholder:text-white/30 tracking-widest" onChange={(e) => setPassword(e.target.value)} />
            <button type="submit" className="w-full bg-white text-slate-900 p-5 rounded-3xl font-black uppercase hover:bg-purple-50 transition-all shadow-xl active:scale-95 tracking-widest">Verifikasi</button>
          </form>
        </motion.div>
      </div>
    );
    ...
    <div className={isDarkMode ? "dark" : ""}>
      <div className="h-screen bg-[#F8FAFF] dark:bg-[#020617] flex text-slate-700 dark:text-slate-200 overflow-hidden transition-all duration-700 font-sans">
        ...
        <aside className="w-72 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 p-8 flex flex-col gap-8 transition-colors">
          ...
          <div className="p-6 bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 rounded-[35px] text-white shadow-xl">
             <p className="text-[10px] font-bold uppercase opacity-70 mb-2 tracking-[0.2em]">{userRole} Access</p>
             <h4 className="text-4xl font-black tracking-tighter">{stats.total}</h4>
             <p className="text-[10px] mt-1 opacity-60 uppercase font-black">Laporan Terarsip</p>
          </div>
          
          <button onClick={() => setIsLoggedIn(false)} className="flex items-center gap-3 p-4 text-slate-400 hover:text-red-400 font-bold rounded-2xl transition-all"><LogOut size={20}/> Logout</button>
        </aside>
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-[#FDFDFF] dark:bg-[#020617] transition-colors">
          <header className="p-8 flex justify-between items-center bg-white/30 dark:bg-slate-900/30 backdrop-blur-md border-b border-slate-100 dark:border-slate-800">
            <div>
              <h1 className="text-2xl font-black text-slate-800 dark:text-white tracking-tighter">Unit Kerja Irban III</h1>
                  ...
                  <motion.div key={file.id} whileHover={{ y: -8 }} className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm p-8 rounded-[45px] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-2xl transition-all group flex flex-col justify-between h-52 cursor-pointer" onClick={() => file.mimeType.includes('folder') ? (setCurrentFolder(file.id), setFolderHistory([...folderHistory, {id: file.id, name: file.name}])) : setSelectedFile(file)}>
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300">
                        {/* Download (Admin & User bisa) */}
                        {!file.mimeType.includes('folder') && (
                          <button onClick={(e) => { e.stopPropagation(); window.open(`https://drive.google.com/uc?export=download&id=${file.id}`, '_blank'); }} className="p-3 bg-white dark:bg-slate-700 rounded-2xl text-slate-400 hover:text-blue-500 shadow-xl border border-slate-50 dark:border-slate-600" title="Unduh Berkas"><Download size={18}/></button>
                        )}
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-black uppercase tracking-widest opacity-40">{file.mimeType.includes('folder') ? 'Directory' : 'PDF Document'}</p>
                    </div>
                  </motion.div>
               ))}
            </div>
        ...
        <AnimatePresence>
          {selectedFile && (
            <motion.div initial={{ x: 600 }} animate={{ x: 0 }} exit={{ x: 600 }} className="w-[550px] bg-white dark:bg-slate-900 shadow-[0_0_50px_rgba(0,0,0,0.1)] border-l border-slate-100 dark:border-slate-800 flex flex-col overflow-hidden relative z-30">
        ...
        <AnimatePresence>
          {isUploadModalOpen && userRole === 'admin' && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white dark:bg-slate-900 rounded-[50px] p-12 w-full max-w-xl relative border border-white/20 shadow-2xl">
                ...
                <p className="text-sm text-slate-400 mb-10 font-medium tracking-wide">Data akan dikirim dengan enkripsi Google Cloud</p>
                <div className="space-y-8">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 ml-4">Target Folder</label>
                    <select value={uploadDestinationId} onChange={(e) => setUploadDestinationId(e.target.value)} className="w-full p-6 rounded-[30px] bg-slate-50 dark:bg-slate-800 border-none outline-none font-bold text-sm text-slate-600 dark:text-slate-300 ring-1 ring-slate-100 dark:ring-slate-700 focus:ring-4 focus:ring-purple-500/20 transition-all appearance-none cursor-pointer">
                  ...
                  <label className="flex flex-col items-center justify-center w-full h-64 border-4 border-dashed border-slate-100 dark:border-slate-800 rounded-[45px] cursor-pointer hover:bg-purple-50/50 dark:hover:bg-purple-900/10 hover:border-purple-200 transition-all text-center p-8 group">
                    ...
                    <p className="text-xs text-slate-400 mt-2 font-medium">Klik untuk mencari di komputer Anda</p>
