...
import { 
  ...
  Crown, Zap, ShieldCheck, Star, Trash2, FileDigit
} from 'lucide-react';
...
export default function Dashboard() {
  // --- STATES (SAMA + TAMBAHAN folderCounts) ---
  ...
  // --- STATE BARU UNTUK HITUNG LAPORAN ---
  ...
  // --- LOGIC BARU: FUNGSI HITUNG ISI FOLDER ---
  const fetchFolderCounts = useCallback(async (fileList: DriveFile[]) => {
    const folders = fileList.filter(f => f.mimeType.includes('folder'));
    folders.forEach(async (folder) => {
      try {
        const res = await fetch(`/api/drive/count?folderId=${folder.id}`);
        const data = await res.json();
        setFolderCounts(prev => ({ ...prev, [folder.id]: data.count }));
      } catch (e) { console.error(e); }
    });
  }, []);
  ...
  // --- DATA FETCHING (DENGAN PEMANGGIL HITUNG) ---
  const fetchData = async (fId: string = '') => {
    ...
    try {
      ...
      if (data.files) { 
        ...
        fetchFolderCounts(data.files); // MEMANGGIL SISTEM HITUNG
      }
    } catch (e) { console.error(e); }
    setLoading(false);
  };

  useEffect(() => { if (isLoggedIn) fetchData(currentFolder); }, [isLoggedIn, currentFolder, fetchFolderCounts]);
      ...
      <div className="relative min-h-screen w-full flex items-center justify-center p-4 bg-[#05070A] font-sans overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 bg-[#0C0F16]/80 backdrop-blur-2xl p-12 rounded-[50px] shadow-[0_0_50px_rgba(245,158,11,0.1)] w-full max-w-md border border-amber-500/20 text-center">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <input required type="text" placeholder="IDENTITAS USER" className="w-full p-5 rounded-2xl border border-amber-500/10 outline-none bg-white/5 text-white text-center font-bold placeholder:text-slate-600 focus:border-amber-500/50 transition-all text-xs uppercase" onChange={(e) => setTempName(e.target.value)} />
      ...
      <div className="h-screen bg-[#F8FAFC] dark:bg-[#05070A] flex text-slate-700 dark:text-slate-200 overflow-hidden font-sans transition-all duration-700">
        ...
        <aside className="w-80 bg-white dark:bg-[#080A0F] border-r border-slate-200 dark:border-amber-500/10 p-8 flex flex-col gap-10 relative z-20">
          ...
          <nav className="flex-1 space-y-2 font-black text-[10px] uppercase tracking-widest">
            ...
            <button onClick={() => setIsLogModalOpen(true)} className="w-full flex items-center gap-4 p-4 text-slate-400 hover:bg-slate-100 dark:hover:bg-amber-500/5 rounded-2xl transition-all mt-4">
                ...
                {filteredFiles.map((file) => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      {/* --- SISTEM PENGHITUNG BARU DI KARTU FOLDER --- */}
                      {file.mimeType.includes('folder') && (
                        <div className="absolute top-8 right-8 flex items-center gap-2 bg-amber-500 text-slate-950 px-3 py-1.5 rounded-full shadow-lg border border-white/20">
                          <FileDigit size={14} />
                          <span className="text-[10px] font-black">{folderCounts[file.id] ?? '...'}</span>
                        </div>
                      )}
        ...
        {/* LOG MODAL, UPLOAD MODAL, PREVIEW, SEARCH TETAP SAMA (DIPASTIKAN LENGKAP) */}
        <AnimatePresence>
          {isLogModalOpen && (
                    ...
                    <tbody className="text-[11px] font-bold">
                      {activityLogs.map((log) => (
                        <tr key={log.id} className="border-b border-slate-50 dark:border-amber-500/5 text-slate-600 dark:text-slate-300 hover:bg-amber-500/5 transition-colors">
                          ...
                          <td className="p-6">
                            <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase text-white ${log.action === 'DELETE' ? 'bg-red-500' : log.action === 'UPLOAD' ? 'bg-green-500' : 'bg-slate-800 dark:bg-amber-500'}`}>
                              {log.action}
                            </span>
                          </td>
                          <td className="p-6 max-w-[200px] truncate opacity-80">{log.fileName}</td>
          ...
          {selectedFile && (
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="w-[750px] bg-white dark:bg-[#080A0F] backdrop-blur-3xl shadow-[-50px_0_100px_rgba(0,0,0,0.4)] border-l border-slate-100 dark:border-amber-500/10 flex flex-col overflow-hidden relative z-30">
               ...
               <div className="flex-1 relative m-10">
                  ...
                  <iframe src={`https://drive.google.com/file/d/${selectedFile.id}/preview`} className="w-full h-full rounded-[50px] overflow-hidden bg-white dark:bg-slate-950 border border-amber-500/10 shadow-2xl transition-all duration-700" title="Live Sync" style={{ opacity: previewLoading ? 0 : 1 }} onLoad={() => setPreviewLoading(false)} />
               </div>
               <div className="p-10 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl border-t border-slate-100 dark:border-amber-500/10">
                 <button onClick={() => { addOnlineLog("DOWNLOAD", selectedFile.name); window.open(`https://drive.google.com/uc?export=download&id=${selectedFile.id}`, '_blank'); }} className="w-full py-7 bg-slate-900 dark:bg-amber-500 text-white dark:text-slate-900 rounded-[30px] font-black text-[11px] uppercase tracking-[0.5em] flex items-center justify-center gap-4 hover:shadow-[0_15px_30px_rgba(245,158,11,0.2)] hover:scale-[1.02] active:scale-95 transition-all shadow-xl"><Download size={22} strokeWidth={4}/> Authorized Retrieval</button>
               </div>
            </motion.div>
          )}
        </AnimatePresence>
